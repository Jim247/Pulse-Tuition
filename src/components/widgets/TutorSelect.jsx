  ...
  return (
    <div className="max-w-7xl mx-auto p-4">
      <div className="max-w-md mx-auto mb-8">
        <select value={selectedInstrument} onChange={handleInstrumentChange} className="w-full p-2 border rounded-md">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {tutorsToDisplay === null ? (
          ...
          tutorsToDisplay.map((tutor) => (
            <div key={tutor.id} className="border rounded-lg shadow-md bg-white overflow-hidden">
              <div className="w-full h-[300px] flex items-center justify-center bg-gray-50">
                <img src={tutor.photo} alt={tutor.name} className="w-full h-full object-contain" />
              </div>
              <div className="p-4">
                <h3 className="font-semibold text-lg mb-2 text-gray-700">{tutor.name}</h3>
                {tutor.id !== 'placeholder' && (
                  <div className="flex gap-2 mb-3">
                    {tutor.instruments.map((instr) => {
                      const data = instruments.find((i) => i.title === instr);
                      return data ? (
                        <img key={instr} src={data.icon} alt={data.title} className="w-8 h-8 object-contain" />
                      ) : null;
                    })}
                  </div>
                )}
                  ...
                  <>
                    <div className="text-sm space-y-2 mb-4">
                      <div className="flex items-center gap-2">
                        <IoCarOutline className={`w-6 h-6 ${tutor.mobile ? 'text-sky-800' : 'text-gray-300'}`} />
                      ...
                      <div className="flex items-center gap-2">
                        <HiOutlineHome className={`w-6 h-6 ${tutor.studio ? 'text-sky-800' : 'text-gray-300'}`} />
